...
function PulseBlock({ className }: { className?: string }) {
  return <div className={cn('animate-pulse rounded-[14px] bg-[#f1f3f5]', className)} />
}

export function PageLoadingState({ label = 'Loading', className }: LoadingStateProps) {
  return (
    <div
      className={cn('mx-auto w-full max-w-[1440px] px-5 py-16 sm:px-8 lg:px-16', className)}
      aria-live="polite"
      aria-busy="true"
    >
      <p className="editable-mono text-[#5a5a5a]">{label}</p>
      <PulseBlock className="mt-6 h-16 w-3/4 max-w-3xl" />
      <PulseBlock className="mt-4 h-5 w-2/3 max-w-2xl" />
      <div className="mt-12 grid gap-5 md:grid-cols-3">
        {[0, 1, 2].map((item) => (
          <div key={item} className="rounded-[18px] border border-[#e5e7eb] p-5">
  ...
  return (
    <div className={cn('grid gap-5 sm:grid-cols-2 lg:grid-cols-3', className)} aria-live="polite" aria-busy="true">
      {Array.from({ length: count }).map((_, index) => (
        <div key={index} className="rounded-[18px] border border-[#e5e7eb] p-4">
...
export function DetailLoadingState({ label = 'Loading', className }: LoadingStateProps) {
  return (
    <div
      className={cn('mx-auto grid w-full max-w-[1440px] gap-10 px-5 py-16 sm:px-8 lg:grid-cols-[0.8fr_1.2fr] lg:px-16', className)}
      aria-live="polite"
      aria-busy="true"
    >
